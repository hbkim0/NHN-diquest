...
export function xmlParser(paramsXml, returnType) {
  ...
  //domParser 실행전 < 특수문자를 &lt;로 치환 작업.
  parserXml = parserXml.replace(/(?<=value=['|"])([<])(?=['|"])/g, "&lt;");
  parserXml = parserXml.replace(/(?<=placeholder=['|"])([<])(?=['|"])/g, "&lt;");
  parserXml = parserXml.replace(/(?<=text=['|"])([<])(?=['|"])/g, "&lt;");
  
  console.log(paramsXml);
  paramsXml = "";

  // iuml 에 오류가 있을경우 일반 텍스트로 변환
  ...
  // xml 정의
  parserXml =
    '<?xml version="1.0" encoding="UTF-8"?>' + "<iuml>" + parserXml + "</iuml>";
  ...
  function isGeneralText(parsedDocument) {
    ...
    if (
      isParseError(
        domParser("<iuml>" + parsedDocument + "</iuml>", "application/xml")
      )
    ) {
    ...
    if (xml.nodeType == 1) {
      ...
      if (xml.nodeName == "text") {
        ...
        for (
          ...
          if (radiobutton_item.nodeName == "data") {
            checkbox.data = radiobutton_item.textContent;
          }
        ...
        if (checkbox.data) {
          obj["@attributes"]["data"] = checkbox.data;
        }
        if (checkbox.dqml) {
          obj["@attributes"]["dqml"] = checkbox.dqml;
        }
        ...
        if (xml.hasChildNodes()) {
          ...
          for (var select_i = 0; select_i < xml.childNodes.length; select_i++) {
            ...
            var select_item_disableYn = "";
            var select_item_disableId = "";
            if (select_item.nodeName == "item") {
              if (select_item.attributes.length > 0) {
                for (
                  ...
                  if (select_item.nodeName.toLowerCase() == "item") {
                    ...
                    if (select_item_attribute.nodeName == "disable-yn") {
                      select_item_disableYn = select_item_attribute.nodeValue;
                    }
                    if (select_item_attribute.nodeName == "disable-id") {
                      select_item_disableId = select_item_attribute.nodeValue;
                    }
                  }
                }
              }
              select.option.push({
                ...
                disableYn: select_item_disableYn,
                disableId: select_item_disableId
              });
            }
          }
          xml.innerHTML = "";
        ...
        if (xml.getAttribute("scroll") == "on") {
          if (xml.hasChildNodes()) {
            for (var lb_i = 0; lb_i < xml.childNodes.length; lb_i++) {
              // console.log('s');
              var layout_bubble_item = xml.childNodes.item(lb_i);
              if (layout_bubble_item.nodeName == "bubble") {
                // console.log('bubble 안에');
                console.log(layout_bubble_item);
                if (layout_bubble_item.getAttribute("style")) {
                  var style = layout_bubble_item.getAttribute("style");
                  if (style.indexOf("width") !== -1) {
                    console.log("width");
                    if (layout_bubble_item.hasChildNodes()) {
                      var nCnt = layout_bubble_item.childNodes.length;
                      for (var lbl_i = 0; lbl_i < nCnt; lbl_i++) {
                        var layout_bubble_layout_item = layout_bubble_item.childNodes.item(
                          lbl_i
                        );
                        if (layout_bubble_layout_item.nodeName == "layout") {
                          console.log("layout_bubble_layout_item");
                          console.log(layout_bubble_layout_item);
                          layout_bubble_layout_item.setAttribute(
                            "style",
                            style
                          );
                          console.log(layout_bubble_layout_item);
      ...
      } else if (xml.nodeName == "layout-paging") {
        // table data 정의 및 하위 요소 삭제
        if (xml.hasChildNodes()) {
          const layoutPage = {
            option: [],
          ...
          for (var page_i = 0; page_i < xml.childNodes.length; page_i++) {
            var page_item = xml.childNodes.item(page_i);
            if (page_item.nodeName == "layout-list") {
              page_item.setAttribute("list_id", page_i);
              layoutPage.total = page_i + 1;
            }
          }
          obj["@attributes"]["total"] = layoutPage.total;
        }
      }
  ...
  for (var i = 0; i < xmlDoc.childNodes.item(0).childNodes.length; i++) {
    if (xmlDoc.childNodes.item(0).childNodes.item(i).nodeName == "#text") {
      var textCheck = xmlDoc.childNodes.item(0).childNodes.item(i).nodeValue;
      textCheck = textCheck.replace(/\s/gi, "");
      if (0 < textCheck.length) {
        var bubbleIumlNode = document.createElement("bubble");
        var textIumlNode = document.createElement("text");
        var text = xmlDoc.childNodes.item(0).childNodes.item(i).nodeValue;
        var textNode = document.createTextNode(text);
        textIumlNode.appendChild(textNode);
        bubbleIumlNode.appendChild(textIumlNode);
        xmlDoc.childNodes.item(0).appendChild(bubbleIumlNode);
      }
    }
  }
  ...
  // 자동 발화 유무 처리
  if (!isButton) {
    json["ans_iuml_0"]["@autoClick"] = true;
  } else {
    json["ans_iuml_0"]["@autoClick"] = false;
  }

  // 이모티콘 정의
  if (emotion) {
    json["ans_iuml_0"]["@attributes"]["emotion"] = emotion;
  }
