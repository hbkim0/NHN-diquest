...
def run_experiment():
    ...
    # 
    dataset_path_root = loader.get_loader_target_dir_root(experiment)

    #
    database.create_experiment_artifacts(experiment)

    #
    try:
        _logger.info('creating a pipeline...')

        loaders = loader.get_loaders(experiment, dataset_path_root)
        trainers = docker.get_trainers(experiment, dataset_path_root)
        
        pipeline = Pipeline(
            name=experiment_id,
            # root='',
            components=loaders + trainers
        )
